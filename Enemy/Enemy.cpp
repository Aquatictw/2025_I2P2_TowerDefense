...
Enemy::Enemy(std::string img, float x, float y, float w, float h, float radius, float speed, float hp, int money) : Engine::Sprite(img, x, y, w, h), speed(speed), hp(hp), money(money)
{
    CollisionRadius = radius;
    reachEndTime = 0;
    timeSinceHit = hitEffectTime; // Initialize to hitEffectTime so no tint at start
}
void Enemy::Hit(float damage)
{
    hp -= damage;
    timeSinceHit = 0; // Reset the hit effect timer
...
void Enemy::Update(float deltaTime)
{
    ...
    {
        // Calculate red tint based on remaining time
